...
def generate_journal(run_id, velocity, aoa_deg, run_dir):
    ...
    Returns path to the generated .jou file.
    """
    os.makedirs(run_dir, exist_ok=True)

    Vx, Vy = decompose_velocity(velocity, aoa_deg)

    # Forward slashes required by Fluent TUI
    case_path   = BASE_CASE.replace("\\", "/")
    run_dir_fwd = run_dir.replace("\\", "/")

    # Output file paths
    cl_cd_file = f"{run_dir_fwd}/cl_cd"
    ...
    # Confirmed TUI sequence from Fluent 2025 R2 log
    ...
    lines.append(f'{velocity}')      # Velocity Magnitude value (m/s)
    lines.append('no')               # Use Profile for Supersonic Gauge Pressure?
    lines.append('0')                # Supersonic/Initial Gauge Pressure (Pa)
    lines.append('no')               # Use Profile for X-Component?
    lines.append(f'{Vx/velocity}')   # X direction component (normalized)
    lines.append('no')               # Use Profile for Y-Component?
    ...
    lines.append(f'{TURB_INTENSITY}')       # Turbulent Intensity (percentage)
    lines.append(f'{TURB_VISC_RATIO}')      # Turbulent Viscosity Ratio
    lines.append('')
    ...
    lines.append('; Set convergence criteria to 1e-6')
    ...
    # ── Solve ─────────────────────────────────────────────────────────────────
    lines.append(f'; Iterate up to {ITERATIONS} steps')
    ...
    lines.append(f'{drag_x}')                  # X force direction
    lines.append(f'{drag_y}')                  # Y force direction
    lines.append(f'yes')                       # write to file?
    lines.append(f'"{cl_cd_file}_cd.txt"')
    lines.append('')
    ...
    lines.append(f'{AIRFOIL_ZONE}')            # zone name
    lines.append(f'()')                        # end of zones
    lines.append(f'{lift_x}')                  # X force direction
    lines.append(f'{lift_y}')                  # Y force direction
    lines.append(f'yes')                       # write to file?
    lines.append(f'"{cl_cd_file}_cl.txt"')
    lines.append('')
    ...
    lines.append(f'{AIRFOIL_ZONE}')
    lines.append(f'()')
    lines.append(f'wall-shear')                # correct field name in Fluent 2025 R2
    lines.append(f'yes')
